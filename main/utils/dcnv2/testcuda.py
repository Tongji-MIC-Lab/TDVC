...
from mmcv.ops.modulated_deform_conv import ModulatedDeformConv2dPack as DCN2
from dcn_v2 import dcn_v2_conv, DCNv2
from dcn_v2 import  DCN as DCN1
...
def example_dconv():
    input = torch.randn(2, 64, 2496, 1472).cuda()
    input = pad(input, 64)
    print(input.shape)
    # wrap all things (offset and mask) in DCN
    dcn = DCN2(64, 64, kernel_size=(3, 3), stride=1,
              padding=1, deformable_groups=2).cuda()
    # print(dcn.weight.shape, input.shape)
    output = dcn(input, input)
