...
modelspath = 'main/utils/pretrained_flownet/'
...
def flow_warp(tensorInput, tensorFlow):
    """
    flow warp
    Inspired from https://github.com/sniklaus/pytorch-pwc
    """
...
class ME_Spynet(nn.Module):
    ...
    def forward(self, im1, im2):
        ...
        zeroshape = [batchsize, 2, shape_fine[2] // 2, shape_fine[3] // 2]  # 为什么是2啊？
        ...
        for intLevel in range(self.L):
            ...
            flowfileds = flowfiledsUpsample + self.moduleBasic[intLevel](torch.cat(
                [im1list[self.L - 1 - intLevel], flow_warp(im2list[self.L - 1 - intLevel], flowfiledsUpsample),
                 flowfiledsUpsample], 1))  # residual flow 输入concat3个warp后的光流图
...
def build_model():
    ...
    # read images
    im1 = cv2.cvtColor(cv2.imread('input.png'), cv2.COLOR_BGR2RGB)
    im1 = im1 / 255.0
    im1 = np.expand_dims(im1, axis=0)
    im2 = cv2.cvtColor(cv2.imread('ref.png'), cv2.COLOR_BGR2RGB)
    im2 = im2 / 255.0
    ...
    rgb_my = cv2.cvtColor(rgb_my, cv2.COLOR_RGB2BGR)
    cv2.imwrite('flow.png', rgb_my)
    ...
    # print(psnr)
    warp_image = cv2.cvtColor(warp_frame[0, :, :, :].transpose(1, 2, 0), cv2.COLOR_RGB2BGR)
    cv2.imwrite('warp2.png', warp_image)
