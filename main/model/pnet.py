    ...
    def forward(self, input_image, refer_frames, enabled_amp, is_compress=False):
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            if is_compress:
                ...
                out_enc = self.mvCoder.compress(estmv.float())
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            if is_compress:
                ...
                out_enc = self.resCoder.compress(input_residual.float())
                ac_bpp_res = sum(len(s[0]) for s in out_enc["strings"]) * 8.0 / num_pixels

        with torch.cuda.amp.autocast(enabled=enabled_amp):
            recon_image = prediction + recon_res
            recon_image = self.loopfilter(recon_image, refer_frames)
            recon_image = recon_image.clamp(0., 1.)

        if self.training:
            return recon_image, bpp_res.view(-1), bpp_mv.view(-1), mv_aux_loss, res_aux_loss
        else:
            return recon_image, bpp_res.view(-1), bpp_mv.view(-1)
